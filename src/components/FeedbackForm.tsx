...
const EMAILJS_PUBLIC_KEYS = {
  Contact: 'VLVlPicG50RX7Zug7',
  Course: 'VLVlPicG50RX7Zug7',
  Article: 'SrTcJ2_VA55t8_KMl',
};
 ...
 * Integrated with EmailJS for delivery.
 */
export const FeedbackForm: React.FC<FeedbackFormProps> = ({ isOpen, onClose, subjectName, type }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const templateParams = {
      ...
      type: type,
      rating: type !== 'Contact' ? rating : 'N/A',
    };
      ...
      {isSubmitted ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isSending}
            className="glow-border w-full py-3 bg-medical-primary hover:bg-medical-secondary text-white font-display font-bold rounded-xl transition-colors flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
            style={{ '--glow-color': 'var(--color-success)' } as any}
          >
            {isSending ? (
              <>
